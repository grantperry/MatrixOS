...
void runModule(s8int (*func)(void)) {
	...
	if (SYSCALL_ENABLED) syscall_monitor_write("[....] ");
	if (!SYSCALL_ENABLED) monitor_write("[....] ");
	...
	if(ret == 0) {
		...
		if (SYSCALL_ENABLED) syscall_monitor_write("done");
		if (!SYSCALL_ENABLED) monitor_write("done");
	}
	if(ret >= 1) {
		monitor_set_cursor_pos(1, y);
		monitor_set_fore_colour(13);
		if (SYSCALL_ENABLED) syscall_monitor_write("warn");
		if (!SYSCALL_ENABLED) monitor_write("warn");
	}
	if(ret <= -1) {
		monitor_set_cursor_pos(1, y);
		monitor_set_fore_colour(4);
		if (SYSCALL_ENABLED) syscall_monitor_write("eror");
		if (!SYSCALL_ENABLED) monitor_write("eror");
	}
